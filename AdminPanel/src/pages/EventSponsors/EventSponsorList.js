...
import AddEventSponsor from "./AddEventSponsor";
import ViewEventSponsor from "./ViewEventSponsor";
import EditEventSponsor from "./EditEventSponsor";
...
const EventSponsorList = () => {
  ...
  const onDeleteButtonClick = (value) => {
    if (value) {
      ...
      fetch(
        'https://harsh7541.pythonanywhere.com/admin1/deletesponsor',
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Event Sponsors" pageTitle="Dashboards" pageLink="/dashboard" />
          <Row>
            <Col lg={12}>
      ...
      {addEventSponsorModal ? (
        <AddEventSponsor
          addEventSponsorModal={addEventSponsorModal}
          onCloseModal={() => setAddEventSponsorModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callSponsorListApi();
            setAddEventSponsorModal(false);
          }}
        />
      ) : null}
