...
const AddEventNews = (props) => {
  ...
  const [isTopNews, setIsTopNews] = useState({
    label: "No",
    value: "No",
  });
  ...
  const submitBtnClk = (e) => {
    ...
    if (title === "") {
      ...
      fetch("https://harsh7541.pythonanywhere.com/admin1/addgeneralnews", requestOptions)
        .then((response) => response.json())
        .then((data) => {
          if (
            data &&
