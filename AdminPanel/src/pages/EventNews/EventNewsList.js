...
import AddEventNews from "./AddEventNews";
import ViewEventNews from "./ViewEventNews";
import EditEventNews from "./EditEventNews";
...
const EventNewsList = () => {
  ...
  const [addEventNewsModal, setAddEventNewsModal] = useState(false);
  const [loading, setloading] = useState(true);
  const [editEventNewsModal, setEditEventNewsModal] = useState(false);
  const [selectedRow, setselectedRow] = useState();
  const [selectedViewRow, setselectedViewRow] = useState();
  const [viewEventNewsModal, setViewEventNewsModal] = useState(false);
  ...
  const newsPermissions = detailedPermissions.news || [];
  ...
  const callNewsListApi = () => {
    ...
    fetch(`https://api.linq-staging-site.com/admin1/generalnews`, requestOptions)
  ...
  const toggleUser = useCallback(() => {
    setAddEventNewsModal(!addEventNewsModal);
  }, [addEventNewsModal]);

  const isEditBtnClick = useCallback(
    (row) => {
      setselectedRow(row);
      setEditEventNewsModal(true);
    },
    []
  );
  ...
  const isViewBtnClick = useCallback((row) => {
    ...
    setViewEventNewsModal(true);
  }, []);

  const newsCol = useMemo(
    () => [
      {
        id: "newsCategoryId",
        header: "News Category",
        accessorKey: "newsCategoryDetails.newsCategory",
        filterable: false,
      },
      {
        id: "publistdateId",
        header: "Publish Date",
        accessorKey: "newsCreatedDate",
        filterable: false,
      },
      {
        id: "isTopNews",
        header: "Top News",
        accessorKey: "isTopNews",
        filterable: false,
      },
      {
        id: "titleId",
  ...
  const onDeleteButtonClick = (value) => {
    if (value) {
      ...
      fetch(
        'https://api.linq-staging-site.com/admin1/deletegeneralnews',
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Event News" pageTitle="Dashboards" pageLink="/dashboard" />
          <Row>
            <Col lg={12}>
      ...
      {addEventNewsModal ? (
        <AddEventNews
          addEventNewsModal={addEventNewsModal}
          onCloseModal={() => setAddEventNewsModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callNewsListApi();
            setAddEventNewsModal(false);
          }}
        />
      ) : null}
      ...
      {viewEventNewsModal ? (
        <ViewEventNews
          row={selectedViewRow}
          viewEventNewsModal={viewEventNewsModal}
          onCloseModal={() => setViewEventNewsModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callNewsListApi();
            setViewEventNewsModal(false);
          }}
        />
      ) : null}
      ...
      {editEventNewsModal ? (
        <EditEventNews
          row={selectedRow}
          editEventNewsModal={editEventNewsModal}
          onCloseModal={() => setEditEventNewsModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callNewsListApi();
            setEditEventNewsModal(false);
          }}
        />
      ) : null}
