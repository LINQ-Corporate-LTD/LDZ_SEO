...
const ViewUserPassRegistration = ({ row, viewModal, onCloseModal }) => {
  ...
  const [attandees, setAttandees] = useState("");
  ...
  useEffect(() => {
    if (row) {
      ...
      setCompany(row?.userCompany);
      setEmail(row?.userEmail);
      setMobile(row?.userMobile);
      ...
      >
        <ModalHeader className="bg-light p-3" toggle={onCloseModal}>
          <h5 className="modal-title" id="exampleModalLabel">
            View User Pass Registration Request
          </h5>
        </ModalHeader>
        <Form action="#">
          <ModalBody>
...
export default ViewUserPassRegistration;
