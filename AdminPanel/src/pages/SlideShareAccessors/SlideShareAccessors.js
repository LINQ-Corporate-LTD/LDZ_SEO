...
import AddSlideShareAccessor from "./AddSlideShareAccess";
...
const SlideShareAccessors = () => {
    const navigate = useNavigate();

    const [slideShareAccessorList, setSlideShareAccessorList] = useState([]);
    const [addSlideShareAccessorModal, setAddSlideShareAccessorModal] = useState(false);
    const [loading, setloading] = useState(true);
    // const [editSlideShareAccessorModal, setEditSlideShareAccessorModal] = useState(false);
    ...
    const navMainPermissions = detailedPermissions.eventslideshareaccessors || [];
    ...
    useEffect(() => {
        callSlideShareAccessorListApi();
        // eslint-disable-next-line
    }, []);

    const callSlideShareAccessorListApi = () => {
        setloading(true);

        const requestOptions = {
            method: "GET",
        };
        fetch(`https://api.linq-staging-site.com/admin1/getslideShareAccess`, requestOptions)
            ...
            .then((data) => {
                ...
                if (data && data.status) {
                    setSlideShareAccessorList(data["slideSharesAccess"]);
    ...
    const toggleUser = useCallback(() => {
        setAddSlideShareAccessorModal(!addSlideShareAccessorModal);
    }, [addSlideShareAccessorModal]);
    ...
    const slideShareAccessorCol = useMemo(
        () => [
            {
                id: "slideShareAccessorEmailId",
                header: "Email Id",
                accessorKey: "email",
                filterable: false,
            },
    ...
    const onDeleteButtonClick = (value) => {
        if (value) {
            ...
            fetch(
                'https://api.linq-staging-site.com/admin1/deleteslideShareAccess',
                requestOptions
            )
                .then((response) => response.json())
                .then((data) => {
    ...
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Slide Share Accessors" pageTitle="Dashboards" pageLink="/dashboard" />
                    <Row>
                        <Col lg={12}>
                            <Card className="file-manager-content w-100 p-3 pt-0">
                                        ...
                                        {canAdd && (
                                            <div className="flex-shrink-0">
                                                <button
                                                    ...
                                                    onClick={toggleUser}
                                                >
                                                    <i className="ri-add-line align-bottom me-1"></i> Add Slide Share Accessor
                                                </button>
                                            </div>
                                        )}
                                    </div>
                                ...
                                <CardBody>
                                    <div className="mt-3">
                                        {slideShareAccessorList && slideShareAccessorList?.length > 0 ? (
                                            <TableContainer
                                                columns={slideShareAccessorCol}
                                                data={slideShareAccessorList}
                                                isGlobalFilter={false}
                                                isAddUserList={false}
                                                apiCallFunction={callSlideShareAccessorListApi}
                                                customPageSize={10}
            ...
            {addSlideShareAccessorModal ? (
                <AddSlideShareAccessor
                    addSlideShareAccessorModal={addSlideShareAccessorModal}
                    onCloseModal={() => setAddSlideShareAccessorModal(false)}
                    onModalSubmitBtnClk={() => {
                        setloading(true);
                        callSlideShareAccessorListApi();
                        setAddSlideShareAccessorModal(false);
                    }}
                />
            ) : null}
...
export default SlideShareAccessors;
