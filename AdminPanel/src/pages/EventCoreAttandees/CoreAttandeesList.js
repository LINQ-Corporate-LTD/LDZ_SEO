...
import AddCoreAttandee from "./AddCoreAttandee";
...
const CoreAttandeesList = () => {
  const navigate = useNavigate();

  const [coreAttandeeList, setCoreAttandeeList] = useState([]);
  const [addCoreAttandeeModal, setAddCoreAttandeeModal] = useState(false);
  const [loading, setloading] = useState(true);
  const [editCoreAttandeeModal, setEditCoreAttandeeModal] = useState(false);
  ...
  const callCoreAttandeeListApi = () => {
    ...
    fetch(`https://api.linq-staging-site.com/admin1/eventcoreattandees`, requestOptions)
  ...
  const toggleUser = useCallback(() => {
    setAddCoreAttandeeModal(!addCoreAttandeeModal);
  }, [addCoreAttandeeModal]);

  const isEditBtnClick = useCallback(
    (row) => {
      setselectedRow(row);
      setEditCoreAttandeeModal(true);
    },
    []
  );
  ...
  const coreAttandeesCol = useMemo(
    () => [
      {
        id: "nameId",
        header: "Core Attendee Name",
        accessorKey: "corAttandeeName",
        filterable: false,
      },
      {
        header: "Action",
  ...
  const onDeleteButtonClick = (value) => {
    if (value) {
      ...
      fetch(
        'https://api.linq-staging-site.com/admin1/deleteeventcoreattandee',
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Event Core Attandees" pageTitle="Dashboards" pageLink="/dashboard" />
          <Row>
            <Col lg={12}>
              <Card className="file-manager-content w-100 p-3 pt-0">
                ...
                <CardBody>
                  <div className="mt-3">
                    {coreAttandeeList && coreAttandeeList?.length > 0 ? (
                      <TableContainer
                        columns={coreAttandeesCol}
                        data={coreAttandeeList}
                        isGlobalFilter={false}
                        isAddUserList={false}
                        apiCallFunction={callCoreAttandeeListApi}
                        customPageSize={10}
      ...
      {addCoreAttandeeModal ? (
        <AddCoreAttandee
          addCoreAttandeeModal={addCoreAttandeeModal}
          onCloseModal={() => setAddCoreAttandeeModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callCoreAttandeeListApi();
            setAddCoreAttandeeModal(false);
          }}
        />
      ) : null}
      ...
      {editCoreAttandeeModal ? (
        <EditCoreAttandee
          row={selectedRow}
          editCoreAttandeeModal={editCoreAttandeeModal}
          onCloseModal={() => setEditCoreAttandeeModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callCoreAttandeeListApi();
            setEditCoreAttandeeModal(false);
          }}
        />
      ) : null}
...
export default CoreAttandeesList;
