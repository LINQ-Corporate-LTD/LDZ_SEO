...
import AddExpertSpeaker from "./AddExpertSpeaker";
import EditExpertSpeaker from "./EditExpertSpeaker";
...
const ExpertSpeakers = () => {
  ...
  const [addExpertSpeakerModal, setAddExpertSpeakerModal] = useState(false);
  const [loading, setloading] = useState(true);
  const [editExpertSpeakerModal, setEditExpertSpeakerModal] = useState(false);
  ...
  const toggleUser = useCallback(() => {
    setAddExpertSpeakerModal(!addExpertSpeakerModal);
  }, [addExpertSpeakerModal]);

  const isEditBtnClick = useCallback(
    (row) => {
      setselectedRow(row);
      setEditExpertSpeakerModal(true);
    },
    []
  );
  ...
  const expertSpeakerCol = useMemo(
    () => [
      {
        id: "speakerName",
        header: "Speaker Name",
        accessorKey: "expertSpeakerName",
        filterable: false,
      },
      {
        id: "companyName",
        header: "Company Name",
        accessorKey: "expertSpeakerCompany",
        filterable: false,
      },
      {
        header: "Action",
  ...
  const onDeleteButtonClick = (value) => {
    if (value) {
      ...
      fetch(
        'https://api.linq-staging-site.com/admin1/deleteexpertspeaker',
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Expert Speakers" pageTitle="Dashboards" pageLink="/dashboard" />
          <Row>
            <Col lg={12}>
              <Card className="file-manager-content w-100 p-3 pt-0">
                ...
                <CardBody>
                  <div className="mt-3">
                    {expertSpeakerList && expertSpeakerList?.length > 0 ? (
                      <TableContainer
                        columns={expertSpeakerCol}
                        data={expertSpeakerList}
                        isGlobalFilter={false}
                        isAddUserList={false}
                        apiCallFunction={callExpertSpeakerListApi}
                        customPageSize={10}
      ...
      {addExpertSpeakerModal ? (
        <AddExpertSpeaker
          addExpertSpeakerModal={addExpertSpeakerModal}
          onCloseModal={() => setAddExpertSpeakerModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callExpertSpeakerListApi();
            setAddExpertSpeakerModal(false);
          }}
        />
      ) : null}

      {editExpertSpeakerModal ? (
        <EditExpertSpeaker
          row={selectedRow}
          editExpertSpeakerModal={editExpertSpeakerModal}
          onCloseModal={() => setEditExpertSpeakerModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callExpertSpeakerListApi();
            setEditExpertSpeakerModal(false);
          }}
        />
      ) : null}
