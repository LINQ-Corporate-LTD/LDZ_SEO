...
const JoinedSponsors = () => {
  const navigate = useNavigate();

  const [sponsoredCompanyList, setSponsoredCompanyList] = useState([]);
  ...
  useEffect(() => {
    callSponsoredCompanyListApi();
    // eslint-disable-next-line
  }, []);

  const callSponsoredCompanyListApi = () => {
    setloading(true);

    const requestOptions = {
      method: "GET",
    };
    fetch(`https://harsh7541.pythonanywhere.com/admin1/joinedsponsorcompanies`, requestOptions)
      ...
      .then((data) => {
        ...
        if (data && data.status) {
          setSponsoredCompanyList(data["joiedSponsorCompanies"]);
    ...
    () => [
      {
        ...
        accessorKey: "relatedPurchaseDetails.sponsorPackageType",
        filterable: false,
      },
      {
        id: "companyNameId",
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Sponsor Companies" pageTitle="Dashboards" pageLink="/dashboard" />
          <Row>
            <Col lg={12}>
              <Card className="file-manager-content w-100 p-3 pt-0">
                <CardHeader className="border-0">
                  <div className="d-flex align-items-center">
                    <div className="flex-grow-1">
                      <h5 className="card-title mb-0 fs-15">Sponsor Company List</h5>
                    </div>
                    {/* {permissions?.create && ( */}
                ...
                <CardBody>
                  <div className="mt-3">
                    {sponsoredCompanyList && sponsoredCompanyList?.length > 0 ? (
                      <TableContainer
                        columns={companyCol}
                        data={sponsoredCompanyList}
                        isGlobalFilter={false}
                        isAddUserList={false}
                        apiCallFunction={callSponsoredCompanyListApi}
                        customPageSize={10}
