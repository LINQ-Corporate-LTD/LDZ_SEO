...
const JoinedSponsorDelegates = () => {
  const navigate = useNavigate();

  const [sponsoredDelegateList, setSponsoredDelegateList] = useState([]);
  ...
  useEffect(() => {
    callSponsoredDelegateListApi();
    // eslint-disable-next-line
  }, []);

  const callSponsoredDelegateListApi = () => {
    setloading(true);

    const requestOptions = {
      method: "GET",
    };
    fetch(`https://harsh7541.pythonanywhere.com/admin1/joinedsponsordelegates`, requestOptions)
      ...
      .then((data) => {
        ...
        if (data && data.status) {
          setSponsoredDelegateList(data["joiedSponsorDelegates"]);
          ...
          <Row>
            <Col lg={12}>
              <Card className="file-manager-content w-100 p-3 pt-0">
                <CardHeader className="border-0">
                  <div className="d-flex align-items-center">
                    <div className="flex-grow-1">
                      <h5 className="card-title mb-0 fs-15">Sponsored Delegate List</h5>
                    </div>
                    {/* {permissions?.create && ( */}
                ...
                <CardBody>
                  <div className="mt-3">
                    {sponsoredDelegateList && sponsoredDelegateList?.length > 0 ? (
                      <TableContainer
                        columns={delegateCol}
                        data={sponsoredDelegateList}
                        isGlobalFilter={false}
                        isAddUserList={false}
                        apiCallFunction={callSponsoredDelegateListApi}
                        customPageSize={10}
...
export default JoinedSponsorDelegates;
