...
const EventCompanyTransections = () => {
  const navigate = useNavigate();

  const [companyTransectionList, setCompanyTransectionList] = useState([]);
  ...
  useEffect(() => {
    callCompanyTransectionListApi();
    // eslint-disable-next-line
  }, []);

  const callCompanyTransectionListApi = () => {
    setloading(true);

    const requestOptions = {
      method: "GET",
    };
    fetch(`https://harsh7541.pythonanywhere.com/admin1/companytransections`, requestOptions)
      ...
      .then((data) => {
        ...
        if (data && data.status) {
          setCompanyTransectionList(data["companyTransections"]);
  ...
  const companytransectionsCol = useMemo(
    () => [
      {
        id: "companyId",
        header: "Company",
        accessorKey: "relatedCompanyDetails.companyName",
        filterable: false,
      },
      {
        id: "invoiceNo",
          ...
          <Row>
            <Col lg={12}>
              <Card className="file-manager-content w-100 p-3 pt-0">
                <CardHeader className="border-0">
                  <div className="d-flex align-items-center">
                    <div className="flex-grow-1">
                      <h5 className="card-title mb-0 fs-15">Company Transections List</h5>
                    </div>
                    {/* {permissions?.create && ( */}
                ...
                <CardBody>
                  <div className="mt-3">
                    {companyTransectionList && companyTransectionList?.length > 0 ? (
                      <TableContainer
                        columns={companytransectionsCol}
                        data={companyTransectionList}
                        isGlobalFilter={false}
                        isAddUserList={false}
                        apiCallFunction={callCompanyTransectionListApi}
                        customPageSize={10}
...
export default EventCompanyTransections;
