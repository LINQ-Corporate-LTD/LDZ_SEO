...
const PayOnlineResponse = () => {
    const navigate = useNavigate();

    const [payOnlineRequestList, setPayOnlineRequestList] = useState([]);
    ...
    const subscriberPermissions = detailedPermissions.payonlinerequest || [];
    const canDelete = subscriberPermissions.includes("delete");

    useEffect(() => {
        callPayOnlineRequestListApi();
        // eslint-disable-next-line
    }, []);

    const callPayOnlineRequestListApi = () => {
        setloading(true);

        const requestOptions = {
            method: "GET",
        };
        fetch(`https://harsh7541.pythonanywhere.com/admin1/payonlinetransections`, requestOptions)
            ...
            .then((data) => {
                ...
                if (data && data.status) {
                    setPayOnlineRequestList(data["payOnlineTransections"]);
    ...
    const payOnlineRequestCol = useMemo(
        () => [
            {
                id: "invoiceId",
                header: "Invoice No.",
    ...
    //             body: finalData,
    //         };
    //         fetch("https://harsh7541.pythonanywhere.com/admin1/deletesubscriber", requestOptions)
    //             .then((response) => response.json())
    //             .then((data) => {
    //                 if (
    //                     data &&
    ...
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Pay Online Response" pageTitle="Dashboards" pageLink="/dashboard" />
                    <Row>
                        <Col lg={12}>
                            <Card className="file-manager-content w-100 p-3 pt-0">
                                ...
                                <CardBody>
                                    <div className="mt-3">
                                        {payOnlineRequestList && payOnlineRequestList?.length > 0 ? (
                                            <TableContainer
                                                columns={payOnlineRequestCol}
                                                data={payOnlineRequestList}
                                                isGlobalFilter={false}
                                                isAddUserList={false}
                                                apiCallFunction={callPayOnlineRequestListApi}
                                                customPageSize={10}
...
export default PayOnlineResponse;
