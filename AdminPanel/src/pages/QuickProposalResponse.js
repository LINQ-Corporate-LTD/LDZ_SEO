...
import ViewQuickProposalResponse from "./ViewQuickProposal";
...
const QuickProposalResponse = () => {
  ...
  const quickPropPermissions = detailedPermissions.quickProposalResponse || [];
  const canView = quickPropPermissions.includes("view");
  const canDelete = quickPropPermissions.includes("delete");
  ...
  const callResponseListApi = () => {
    ...
    fetch(`https://api.linq-staging-site.com/admin1/quickproposalresponses`, requestOptions)
      ...
      .then((data) => {
        ...
        if (data && data.status) {
          setResponseList(data["quickProposalList"]);
  ...
  const onDeleteButtonClick = (value) => {
    if (value) {
      ...
      fetch(
        "https://api.linq-staging-site.com/admin1/deletequickproposalrequest",
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
...
export default QuickProposalResponse;
