...
import AddOfferCoupon from "./AddOfferCoupon";
...
const OfferCouponList = () => {
  const navigate = useNavigate();

  const [offerCouponList, setOfferCouponList] = useState([]);
  const [addOfferCouponModal, setAddOfferCouponModal] = useState(false);
  const [loading, setloading] = useState(true);
  const [editOfferCouponModal, setEditOfferCouponModal] = useState(false);
  ...
  useEffect(() => {
    callOfferCouponListApi();
    // eslint-disable-next-line
  }, []);

  const callOfferCouponListApi = () => {
    setloading(true);

    const requestOptions = {
      method: "GET",
    };
    fetch(`https://harsh7541.pythonanywhere.com/admin1/offercoupons`, requestOptions)
      ...
      .then((data) => {
        ...
        if (data && data.status) {
          setOfferCouponList(data["offerCoupons"]);
  ...
  const toggleUser = useCallback(() => {
    setAddOfferCouponModal(!addOfferCouponModal);
  }, [addOfferCouponModal]);

  const isEditBtnClick = useCallback(
    (row) => {
      setselectedRow(row);
      setEditOfferCouponModal(true);
    },
    []
  );
  ...
  const onDeleteButtonClick = (value) => {
    if (value) {
      ...
      fetch(
        'https://harsh7541.pythonanywhere.com/admin1/deleteoffercoupon',
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  return (
    <React.Fragment>
          ...
          <Row>
            <Col lg={12}>
              <Card className="file-manager-content w-100 p-3 pt-0">
                ...
                <CardBody>
                  <div className="mt-3">
                    {offerCouponList && offerCouponList?.length > 0 ? (
                      <TableContainer
                        columns={offercouponCol}
                        data={offerCouponList}
                        isGlobalFilter={false}
                        isAddUserList={false}
                        apiCallFunction={callOfferCouponListApi}
                        customPageSize={10}
      ...
      {addOfferCouponModal ? (
        <AddOfferCoupon
          addOfferCouponModal={addOfferCouponModal}
          onCloseModal={() => setAddOfferCouponModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callOfferCouponListApi();
            setAddOfferCouponModal(false);
          }}
        />
      ) : null}
      ...
      {editOfferCouponModal ? (
        <EditOfferCoupon
          row={selectedRow}
          editOfferCouponModal={editOfferCouponModal}
          onCloseModal={() => setEditOfferCouponModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callOfferCouponListApi();
            setEditOfferCouponModal(false);
          }}
        />
      ) : null}
...
export default OfferCouponList;
