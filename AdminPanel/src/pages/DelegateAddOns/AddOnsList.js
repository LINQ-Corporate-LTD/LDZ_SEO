...
import AddModel from "./AddAddOns";
import EditModel from "./EditAddOns";
...
const AddOnsList = () => {
  ...
  const callAddOnsListApi = () => {
    setloading(true);

    const requestOptions = {
      method: "GET",
    };
    fetch(`https://harsh7541.pythonanywhere.com/admin1/delegatepackageaddons`, requestOptions)
      ...
      .then((data) => {
        ...
        if (data && data.status) {
          setAddOnsList(data["delegateAddOns"]);
  ...
  const addOnCol = useMemo(
    () => [
      {
        id: "title",
        header: "Title",
        accessorKey: "addOnPointName",
        filterable: false,
      },
      {
        id: "price",
  ...
  const onDeleteButtonClick = (value) => {
    if (value) {
      ...
      fetch(
        'https://harsh7541.pythonanywhere.com/admin1/deletedelegatepackageaddons',
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Delegate Add Ons" pageTitle="Dashboards" pageLink="/dashboard" />
          <Row>
            <Col lg={12}>
                  ...
                  <div className="mt-3">
                    {addOnsList && addOnsList?.length > 0 ? (
                      <TableContainer
                        ...
                        isAddUserList={false}
                        apiCallFunction={callAddOnsListApi}
                        customPageSize={10}
