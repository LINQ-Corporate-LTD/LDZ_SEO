...
const NewsCategoryList = () => {
  ...
  const [addNewsCategoryModal, setAddNewsCategoryModal] = useState(false);
  const [loading, setloading] = useState(true);
  const [editNewsCategoryModal, setEditNewsCategoryModal] = useState(false);
  ...
  const callNewsCategoryListApi = () => {
    ...
    fetch(`https://harsh7541.pythonanywhere.com/admin1/newscategories`, requestOptions)
  ...
  const toggleUser = useCallback(() => {
    setAddNewsCategoryModal(!addNewsCategoryModal);
  }, [addNewsCategoryModal]);

  const isEditBtnClick = useCallback(
    (row) => {
      setselectedRow(row);
      setEditNewsCategoryModal(true);
    },
    []
  );
  ...
  const onDeleteButtonClick = (value) => {
    if (value) {
      ...
      fetch(
        'https://harsh7541.pythonanywhere.com/admin1/deletenewscategory',
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
    ...
    <React.Fragment>
                  ...
                  <div className="mt-3">
                    {newsCategoryList && newsCategoryList?.length > 0 ? (
                      <TableContainer
                        ...
                        isAddUserList={false}
                        apiCallFunction={callNewsCategoryListApi}
                        customPageSize={10}
      ...
      {addNewsCategoryModal ? (
        <AddNewsCategory
          ...
          onCloseModal={() => setAddNewsCategoryModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callNewsCategoryListApi();
            setAddNewsCategoryModal(false);
          }}
        />
      ) : null}
      ...
      {editNewsCategoryModal ? (
        <EditNewsCategory
          row={selectedRow}
          editNewsCategoryModal={editNewsCategoryModal}
          onCloseModal={() => setEditNewsCategoryModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callNewsCategoryListApi();
            setEditNewsCategoryModal(false);
          }}
        />
      ) : null}
