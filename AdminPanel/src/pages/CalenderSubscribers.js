...
const CalenderSubscribers = () => {
  const navigate = useNavigate();

  const [calendersubscriberList, setCalenderSubscriberList] = useState([]);
  ...
  const subscriberPermissions = detailedPermissions.calendersubscribers || [];
  const canDelete = subscriberPermissions.includes("delete");

  useEffect(() => {
    callCalenderSubscriberListApi();
    // eslint-disable-next-line
  }, []);

  const callCalenderSubscriberListApi = () => {
    setloading(true);

    const requestOptions = {
      method: "GET",
    };
    fetch(`https://harsh7541.pythonanywhere.com/admin1/calendersubscribers`, requestOptions)
      ...
      .then((data) => {
        ...
        if (data && data.status) {
          setCalenderSubscriberList(data["calenderSubscribers"]);
  ...
  const calenderSubscriberCol = useMemo(
    () => [
      {
        id: "name",
        header: "Calender Subscriber",
        accessorKey: "calenderSubscriber",
        filterable: false,
      },
      {
        id: "createTime",
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Event Calender Subscribers" pageTitle="Dashboards" pageLink="/dashboard" />
          <Row>
            <Col lg={12}>
              <Card className="file-manager-content w-100 p-3 pt-0">
                ...
                <CardBody>
                  <div className="mt-3">
                    {calendersubscriberList && calendersubscriberList?.length > 0 ? (
                      <TableContainer
                        columns={calenderSubscriberCol}
                        data={calendersubscriberList}
                        isGlobalFilter={false}
                        isAddUserList={false}
                        apiCallFunction={callCalenderSubscriberListApi}
                        customPageSize={10}
...
export default CalenderSubscribers;
