...
import AddDelegatePackage from "./AddDelegatePackage";
...
const DelegatePackageList = () => {
  const navigate = useNavigate();

  const [delegatePackageList, setDelegatePackageList] = useState([]);
  const [addDelegatePackageModal, setAddDelegatePackageModal] = useState(false);
  const [loading, setloading] = useState(true);
  const [editDelegatePackageModal, setEditDelegatePackageModal] =
    useState(false);
  ...
  const toggleUser = useCallback(() => {
    setAddDelegatePackageModal(!addDelegatePackageModal);
  }, [addDelegatePackageModal]);

  const isEditBtnClick = useCallback((row) => {
    setselectedRow(row);
    setEditDelegatePackageModal(true);
  }, []);
  ...
  const delPackageCol = useMemo(
    () => [
      {
        id: "delPackageId",
        header: "Package",
        accessorKey: "deligatePackageName",
        filterable: false,
      },
      {
        id: "delPackagePrice",
        header: "Package Price",
        accessorKey: "deligatePackagePrice",
        filterable: false,
      },
      {
        id: "delPackageStatus",
        header: "Package Status",
        accessorKey: "deligatePackageStatus",
        filterable: false,
      },
      {
        id: "showOrderId",
        header: "Show Order",
        accessorKey: "deligatePackageShowOrder",
        filterable: false,
      },
      {
        id: "delPackageExpiry",
        header: "Expiry Date",
        accessorKey: "deligatePackageExpiryDate",
        filterable: false,
        cell: ({ getValue }) => {
  ...
  const onDeleteButtonClick = (value) => {
    if (value) {
      ...
      fetch(
        "https://linq-staging-site.com/admin1/deletedelegatepackage",
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  return (
    <React.Fragment>
          ...
          <Row>
            <Col lg={12}>
              <Card className="file-manager-content w-100 p-3 pt-0">
                ...
                <CardBody>
                  <div className="mt-3">
                    {delegatePackageList && delegatePackageList?.length > 0 ? (
                      <TableContainer
                        columns={delPackageCol}
                        data={delegatePackageList}
                        isGlobalFilter={false}
                        isAddUserList={false}
                        apiCallFunction={callDelegatePackageListApi}
                        customPageSize={10}
      ...
      {addDelegatePackageModal ? (
        <AddDelegatePackage
          addDelegatePackageModal={addDelegatePackageModal}
          onCloseModal={() => setAddDelegatePackageModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callDelegatePackageListApi();
            setAddDelegatePackageModal(false);
          }}
        />
      ) : null}
      ...
      {editDelegatePackageModal ? (
        <EditDelegatePackage
          row={selectedRow}
          editDelegatePackageModal={editDelegatePackageModal}
          onCloseModal={() => setEditDelegatePackageModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callDelegatePackageListApi();
            setEditDelegatePackageModal(false);
          }}
        />
      ) : null}
...
export default DelegatePackageList;
