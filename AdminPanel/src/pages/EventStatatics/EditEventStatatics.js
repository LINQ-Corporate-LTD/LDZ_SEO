...
const EditEventStatatics = ({
  ...
  useEffect(() => {
    if (row) {
      setCountTopic(row?.topicLabel);
      setCount(row?.topicCount);
      setPlusEnable(row.countIcon === "true");
    }
  }, [location]);
  ...
  const submitBtnClk = (e) => {
    ...
    if (countTopic === "") {
      ...
      fetch("https://harsh7541.pythonanywhere.com/admin1/edithomecountpoints", requestOptions)
  ...
  return (
    <>
      <Modal
        id="showModal"
        size="lg"
        isOpen={editEventStataticsModal}
        toggle={onCloseModal}
        centered
      >
...
export default EditEventStatatics;
