...
import AddEventProject from "./AddEventProject";
...
const EventProjects = () => {
  ...
  const [eventProjectList, setEventProjectList] = useState([]);
  const [addEventProjectModal, setAddEventProjectModal] = useState(false);
  const [loading, setloading] = useState(true);
  const [editEventProjectModal, setEditEventProjectModal] = useState(false);
  ...
  useEffect(() => {
    callEventProjectListApi();
    // eslint-disable-next-line
  }, []);

  const callEventProjectListApi = () => {
    setloading(true);

    const requestOptions = {
      method: "GET",
    };
    fetch(`https://api.linq-staging-site.com/admin1/eventprojects`, requestOptions)
      ...
      .then((data) => {
        ...
        if (data && data.status) {
          setEventProjectList(data["eventProjects"]);
  ...
  const toggleUser = useCallback(() => {
    setAddEventProjectModal(!addEventProjectModal);
  }, [addEventProjectModal]);

  const isEditBtnClick = useCallback(
    (row) => {
      setselectedRow(row);
      setEditEventProjectModal(true);
    },
    []
  );
  ...
  const onDeleteButtonClick = (value) => {
    if (value) {
      ...
      fetch(
        'https://api.linq-staging-site.com/admin1/deleteeventproject',
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Event Projects" pageTitle="Dashboards" pageLink="/dashboard" />
          <Row>
            <Col lg={12}>
              <Card className="file-manager-content w-100 p-3 pt-0">
                ...
                <CardBody>
                  <div className="mt-3">
                    {eventProjectList && eventProjectList?.length > 0 ? (
                      <TableContainer
                        columns={projetCol}
                        data={eventProjectList}
                        isGlobalFilter={false}
                        isAddUserList={false}
                        apiCallFunction={callEventProjectListApi}
                        customPageSize={10}
      ...
      {addEventProjectModal ? (
        <AddEventProject
          addEventProjectModal={addEventProjectModal}
          onCloseModal={() => setAddEventProjectModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callEventProjectListApi();
            setAddEventProjectModal(false);
          }}
        />
      ) : null}

      {editEventProjectModal ? (
        <EditEventProject
          row={selectedRow}
          editEventProjectModal={editEventProjectModal}
          onCloseModal={() => setEditEventProjectModal(false)}
          onModalSubmitBtnClk={() => {
            setloading(true);
            callEventProjectListApi();
            setEditEventProjectModal(false);
          }}
        />
      ) : null}
